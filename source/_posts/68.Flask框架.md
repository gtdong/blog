...
```python
Copyfrom werkzeug.wrappers import Request, Response
...
```python
Copy@app.route('/detail/<int:nid>',methods=['GET'],endpoint='detail')
```

### 默认转换器

```python
CopyDEFAULT_CONVERTERS = {
...
```python
Copy@app.route和app.add_url_rule参数:
rule, URL规则
view_func, 视图函数名称
...
```python
Copyfrom flask import Flask,render_template,Markup,jsonify,make_response
app = Flask(__name__)
...
```python
Copyfrom flask import Flask,flash,get_flashed_messages,request,redirect
...
```python
Copyctx = self.request_context(environ)
        error = None
        try:
            try:
                ctx.push()
...
```python
Copyfrom flask_test import  app
if __name__ == '__main__':
    app.run(port=8008)
```
...
```python
Copyfrom flask import Blueprint
...
```python
Copy第一阶段：将ctx(request,session)放到Local对象上
...
```python
Copysession对象是可以跨request的，只要session还未失效，不同的request的请求会获取到同一个session，但是g对象不是，g对象不需要管过期时间，请求一次就g对象就改变了一次，或者重新赋值了一次
```
...
```python
Copyfrom flask import Flask,session
...
```python
Copyresponse.set_cookie('k','v',exipre=None)#这样设置即可
#在session中设置
...
```python
Copy#源码expires = self.get_expiration_time(app, session)
...
```python
Copyobj = SQLHelper.fetch_one("select id,name from users where name=%(user)s and pwd=%(pwd)s", form.data)
```
...
```python
Copyfrom flask import Flask, render_template, request, redirect
...
```python
Copyrequest_started = _signals.signal('request-started')                # 请求到来前执行
...
```python
Copyfrom flask import Flask,signals,render_template
...
```python
Copyfrom flask import Flask, current_app, flash, render_template
...
```python
Copyfrom werkzeug.wsgi import DispatcherMiddleware
from werkzeug.serving import run_simple
...
```python
Copyfrom flask_script import Manager
app = Flask(__name__)
manager=Manager(app)
...
if __name__ == '__main__':
